/**
 * Migration: Add English fields to tours and categories
 * Run once after database is initialized.
 */
...
console.log('Running English data migration...');
...
for (const col of tourColumns) {
  try {
    ...
    console.log(`  Added tours.${col.split(' ')[0]}`);
  } catch (e) {
...
try {
  ...
  console.log('  Added categories.name_en');
} catch (e) {}
...
console.log(`  Updated ${catUpdates.length} categories`);

// Tour English data: [id, title_en, type_label_en, description_en, transport_en, highlights_en, route_en]
const toursEn = [
  ...
  [6, 'Bali 6-Day Semi Self-Guided Tour', 'International', 'Lembongan Island, private villa, Banyan Tree afternoon tea', 'Direct flight from Shanghai', 'Lembongan|Ubud Palace|Jimbaran Beach|Tanah Lot', 'Shanghai-Bali-Lembongan-Bali-Shanghai'],
  ...
  [19, 'Forbidden City 3-Day In-Depth Tour', 'Domestic Short', '600-year Forbidden City in-depth tour. Three Great Halls, Imperial Garden, Jingshan Park view.', 'High-speed rail to Beijing + subway', 'Hall of Supreme Harmony|Palace of Heavenly Purity|Imperial Garden|Jingshan Park|National Museum', 'Beijing-Forbidden City-Jingshan Park-National Museum-Tiananmen Square-Return'],
];
...
for (const t of toursEn) {
  const [id, titleEn, typeEn, descEn, transEn, highEn, routeEn] = t;
  const result = updateTour.run(titleEn, typeEn, descEn, transEn, highEn, routeEn, id);
  if (result.changes > 0) count++;
}
console.log(`  Updated ${count} tours with English data`);
